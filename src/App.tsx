...
const App = () => {
	...
	const handleSubmit = (e: any) => {
		...
		if (
			...
			setError(
				"Vaqt formati noto'g'ri! HH:MM formatida kiriting (masalan: 09:00)"
			);
			return;
		}
	...
	const handleDelete = () => {
		setWorkDays((prev: any) =>
			prev.filter((day: any) => day.id !== selectedId)
		);
		setSelectedId(null);
	};

	const totalHours = workDays.reduce(
		(sum: any, day: any) => sum + day.hours,
		0
	);

	return (
		<div className="max-w-4xl mx-auto p-4 py-32">
			<div className="bg-white rounded-lg shadow-md mb-6 p-6 border">
				...
				<form onSubmit={handleSubmit} className="space-y-4">
					...
					<Button
						type="submit"
						className="w-full bg-blue-500 text-center text-white py-2 rounded-md hover:bg-blue-600 transition-colors"
					>
						Qo'shish
					</Button>
				</form>
					...
					<table className="w-full">
						...
						<tbody className="divide-y divide-gray-200">
							{workDays.map((day: any) => (
								<tr key={day.id} className="hover:bg-[#f0f1f3] translate-all duration-200">
